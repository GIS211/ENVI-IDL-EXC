...
pro image_merge_test
  ...
  for indx1 = 0 , k-1 do begin;文件循环
    ...
    temp_C2 = base_path + 'MODIS\' + v_c1_basena[indx1+x] + '.hdr'
    temp_fmask2 = base_path + 'fmask\' + v_mask_basena[indx1+x] + '.hdr'
...
;      predicted_data2 = fine1[*,*,1]
      ...
      for fmask_index_arr = 0 , fmask_index.length-1 do begin ;像元循环
        d = array_indices(predicted_data,fmask_index[fmask_index_arr])
        sample = d[0]
        line = d[1]
        ...
        ;这里接下来应该是循环行列
        ;for  sample = index_col+(window_size+1)/2-1 , ns- (window_size+1)/2 do begin
        ;for  line = index_row+(window_size+1)/2-1 , nl- (window_size+1)/2 do begin
        ;获取行列位置

        ;取像元，开窗口
        ;--------------------------------------------------------------------------------------------------------
        if sample lt 15 then begin
          sample = 15
        endif

        if line lt 15 then begin
          line = 15
        endif

        if sample gt N_elements(predicted_data[*,0])-15 then begin
          sample = N_elements(predicted_data[*,0])-15
        endif

        if line gt N_elements(predicted_data[0,*])-15 then begin
          line = N_elements(predicted_data[0,*])-15
        endif

        data1 = coarse2_data[(sample-(window_size-1)/2):(sample+(window_size-1)/2),(line-(window_size-1)/2):(line+(window_size-1)/2)]
        data2 = fine_input[(sample-(window_size-1)/2):(sample+(window_size-1)/2),(line-(window_size-1)/2):(line+(window_size-1)/2)]
        data3 = coarse1_data[(sample-(window_size-1)/2):(sample+(window_size-1)/2),(line-(window_size-1)/2):(line+(window_size-1)/2)]
        result = 0
        judge = 0
        ;筛选像元
        ;--------------------------------------------------------------------------------------------------------
        ;方差
        similar_pixel_measure = stddev(data2)/class_num;1方差分四个等级，分为四类
        similar_pixel = fltarr(window_size,window_size);相似像元的范围
        if similar_pixel_measure ne 0 then begin;如果相似像元分级不为0
          similar_pixel[where((data2-data2[(window_size-1)/2,(window_size-1)/2]) lt similar_pixel_measure)] = 1;所有小于阈值的地方都为1
          ;阈值以内的内容视为相似像元的值
        ...
        r_center_LM = (data1[(window_size-1)/2,(window_size-1)/2] - data2[(window_size-1)/2,(window_size-1)/2]);中心像元的差别Tijk
        r_center_MM = (data1[(window_size-1)/2,(window_size-1)/2] - data3[(window_size-1)/2,(window_size-1)/2]);Sijk
        ;注释：
