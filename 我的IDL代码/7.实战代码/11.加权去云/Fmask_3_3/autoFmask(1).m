...
% 
% Requirements:
% 1. Use the original Landat TIF images as inputs and each folder only put
% one image. 只允许有一幅影像
% 2. It needs approximately 4G memory to run this algorithm.运行需要至少4G内存
...
main_dir = 'G:\new';
sub_dir =  dir( main_dir );   % 先确定子文件夹
 
for i = 1 : length( sub_dir )
    if( isequal( sub_dir( i ).name, '.' ) || ...
        isequal( sub_dir( i ).name, '..' ) || ...
        ~sub_dir( i ).isdir )   % 如果不是目录跳过
        continue;
    end
     
    sub_dir_path = fullfile( main_dir, sub_dir( i ).name, '*_MTL.txt' );
    images = dir( sub_dir_path );   % 在这个子文件夹下找后缀为jpg的文件

fprintf('Fmask 3.3.1 version start ...\n');
norMTL=images;
existMTL=size(norMTL);
cd_file_path=fullfile( main_dir, sub_dir( i ).name);
cd(cd_file_path);
if existMTL(1)==0
