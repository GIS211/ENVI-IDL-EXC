...
pro stars_new2
  ...
  ;********************************************
  T=T_CVF(0.05,free_degree);two tailed test
  x_star=fltarr(time)
  ;get files matrix
  ...
  for file_num = 0,fnum - 1 do begin;循环有效文件
    GetData,ImgData=fine,ns = ns,nl = nl,nb = nb,Data_Type = Data_Type, FileName = FileName[file_num],Fid = Fid,Map_info = map_Info;插值目标
    fine=float(fine)
    
    ;判断是否有数据，没有数据直接进行输出
    if total(fine[*,*,0],/nan) eq 0 then begin

      print,'contine!'
      for tt=0,9 do begin
        out_name='B:\stars分块\'+STRTRIM(string(tt+1),2) + '\RSI_geotif\'+F1_basena[file_num]
        ENVI_WRITE_ENVI_FILE,/NO_OPEN, fine[*,*,tt], out_name=out_name, /nocopy, $
          ns=ns,nl=nl,nb=1, offset=offset, bnames=bnames, map_info=map_info
      endfor

      for tt=0,9 do begin
        out_name='B:\stars分块\'+STRTRIM(string(tt+1),2) + '\meanr_geotif\'+F1_basena[file_num]
        ENVI_WRITE_ENVI_FILE,/NO_OPEN, fine[*,*,tt], out_name=out_name, /nocopy, $
          ns=ns,nl=nl,nb=1, offset=offset, bnames=bnames, map_info=map_info
      endfor

      for tt=0,9 do begin
        out_name='B:\stars分块\'+STRTRIM(string(tt+1),2) + '\year_geotif\'+F1_basena[file_num]
        ENVI_WRITE_ENVI_FILE,/NO_OPEN, fine[*,*,tt], out_name=out_name, /nocopy, $
          ns=ns,nl=nl,nb=1, offset=offset, bnames=bnames, map_info=map_info
    ...
    RSI_result=fltarr(ns,nl,10)
    meanr=fltarr(ns,nl,10)
    year_index=fltarr(ns,nl,10)
    ...
    for n_col = 0, ns-1 do begin;列循环
      for n_row = 0, nl-1 do begin;行循环
        ...
        if st eq 1 then begin ;failed
          ...
          RSI_result[n_col,n_row,p]=RSI[0,0,k_ind-1];将稳态转移指数存在稳态转移那一年的图像当中
          year_index[n_col,n_row,p]=1995+n_year+1;n_year+1年发生转变
          p=p+1
    ...
    for tt=0,9 do begin
      out_name='B:\stars分块\'+STRTRIM(string(tt+1),2) + '\RSI_geotif\'+F1_basena[file_num]
      ENVI_WRITE_ENVI_FILE,/NO_OPEN, RSI_result[*,*,tt], out_name=out_name, /nocopy, $
        ns=ns,nl=nl,nb=1, offset=offset, bnames=bnames, map_info=map_info
    endfor

    for tt=0,9 do begin
      out_name='B:\stars分块\'+STRTRIM(string(tt+1),2) + '\meanr_geotif\'+F1_basena[file_num]
      ENVI_WRITE_ENVI_FILE,/NO_OPEN, meanr[*,*,tt], out_name=out_name, /nocopy, $
        ns=ns,nl=nl,nb=1, offset=offset, bnames=bnames, map_info=map_info
    endfor

    for tt=0,9 do begin
      out_name='B:\stars分块\'+STRTRIM(string(tt+1),2) + '\year_geotif\'+F1_basena[file_num]
      ENVI_WRITE_ENVI_FILE,/NO_OPEN, year_index[*,*,tt], out_name=out_name, /nocopy, $
        ns=ns,nl=nl,nb=1, offset=offset, bnames=bnames, map_info=map_info
    endfor
