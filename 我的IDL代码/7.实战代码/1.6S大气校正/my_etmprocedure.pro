;该代码用于处理去条带后的ETM+数据
;通过读取去条带后的ETM+数据的HDR影像文件和MTL.TXT的源数据头文件实现对条带修复后的ETM+数据进行大气校正

;2016.7.15 debug the 6s model wrong ETM+ parameter 138 to correct 137
...
pro my_ETMprocedure
  ...
  CD,work_dir
  ;search gap_fill image .hdr
  ETMfiles=FILE_SEARCH(work_dir,'*.HDR',count=fnum)
  fbasena=FILE_BASENAME(ETMfiles)
  ;search head files MTL.TXT
  ...
  for i=0,fnum-1 do begin;总循环输出in文件
    data_year1=strtrim(fix(strmid(fbasena[i],9,7)),2);当前操作数据年份（整型）
    image_files=ETMfiles[i]
    Firs3Chr=strmid(fbasena[i],0,3);前三个字符，用来判断传感器类型
    ;    打开文件以及获取fid
    e=envi()
...
pro my_6S_CORRECT,in_files,fid,i,data_year,MAP_INFO,ns,nl,nb,dims,Firs3Chr,head_basena
  ...
  for T=0,nb-1 do begin;循环波段进行辐射定标
    ...
    tmg=FLTARR(6,1)
    tmb=FLTARR(6,1)
    tmg[0,0] = strtrim(RADIANCE_MULT_BAND_1_value,2)
    ...
    tmb[0,0] = strtrim(RADIANCE_ADD_BAND_1_value,2)
  ...
  REF_OUT='G:\'+strmid(head_basena[i],3,9)+data_year
